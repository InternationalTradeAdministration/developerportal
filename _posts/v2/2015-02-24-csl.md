...
It works by expanding queries to include search terms that are **fuzzily** similar to the given term, where similarity 
is defined by **distance** - the number of changes required to get from one term another. 
...
| 90 | Distance = 1 | The entire query string matches at a distance of 1. |
| 85 | Distance = 1+ | Each word in the query string matches at a distance of 1. |
| 80 | Distance = 2 | The entire query string matches at a distance of 2. |
| 75 | Distance = 2+ | Each word in the query string matches at a distance of 2. |
...
**Note:**  Special characters such as hashtags (#) and ampersands (&) contained in a need to be URL encoded; e.g., *AT&T* should be encoded as *at%26t*.
