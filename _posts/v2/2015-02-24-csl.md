...
Fuzzy name matching enables users to query a name and get usable results without knowing the exact spelling.  The results include a **score** and the higher the score, the more likely the query matches a name in the consolidated screening list.  Scores are computed by calculating the [Damerau–Levenshtein distance](http://en.wikipedia.org/wiki/Damerau%E2%80%93Levenshtein_distance) between the query and the names in the list.

This is important for users who are searching for a name but do not know how the name is saved in the list.  Especially when most of those names have been translated from foreign languages.

This parameter works in tandem with the *name* query.  Set *fuzzy_name=true* to utilize fuzzy matching when searching against the **name** and **alt_names** fields.

    {{ site.webservices_baseurl }}/v2/consolidated_screening_list/search?api_key={your key}&fuzzy_name=true&name={query}

Queries return one of the following values for **score**:

| Score | Rule | Explanation |
|-------|------|-------------|
| 100 | Exact match | The entire query string and each word in the string return an exact match. |
| 95 | Near match | Each word in the string matches but there may be an extra unknown character such as “>”. |
| 90 | Distance = 1 | The words in the query string match at a Damerau-Levenstein distance of 1. |
| 85 | Distance = 1+ | The words in the query string match at a distance of 1 and there may be an extra character. |
| 80 | Distance = 2 | The words in the query string match at a distance of 2. |
| 75 | Distance = 2+ | The words in the query string match at a distance of 2 and there may be an extra character. |
| <75 | Distance = 3 or more | No results returned. |
