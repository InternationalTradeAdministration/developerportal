...
Searches for a match within the **name**, **alt_names**, **remarks**, and **title** fields from all lists.
...
## The Screening Lists, Their Sources and Specific Fields
...
ITA’s data services platform imports each of the screening lists once every hour.  It then consolidates the lists based on common field types such as name, federal_register_notice, and start_date.  Not all lists contain the same fields so some search results may include more detail than others depending upon the Source agency.  Once the data services platform consolidates the lists, it publishes the [CSL API endpoint](http://api.govwizely.com/consolidated_screening_list/search).
