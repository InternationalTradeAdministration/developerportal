...
Each list is imported into the API every night unless the data in the list has not been updated.  If a list has changed since the previous night, then the list is imported and its *last_imported* field is updated.  Its *source_last_updated* is also updated to reflect the date and time it was updated last.

If not, then the list is not imported.  The *source_last_updated* and the *last_imported* fields are not updated.
